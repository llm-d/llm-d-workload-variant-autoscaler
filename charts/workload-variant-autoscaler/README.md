...
| wva.scaleToZero | bool | `false` | Enable scaling variants to zero replicas when idle. Requires HPA's `HPAScaleToZero` feature gate (K8s 1.31+ alpha) or KEDA. See [Scale-to-Zero Guide](../../docs/user-guide/scale-to-zero.md) for configuration details |
